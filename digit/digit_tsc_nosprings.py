...
# filename = "tsc_wrapper.cpython-38-x86_ls64-linux-gnu.so"
...
DEFAULT_CAMERA_CONFIG = {
	"trackbodyid": 1,
	"distance": 4.0,
	# "lookat": np.array((5.0, 5.0, 10.15)),
	# "elevation": -90.0,
	"lookat": np.array((0.0, -2.0, 2.0)),
	"elevation": -45.0,
}
...
class DigitEnv(MujocoEnv, utils.EzPickle):    
	...
	def __init__(self, dynamics_randomization=False, **kwargs):
		...
		self.simrate = 10 # ms simulate X mujoco steps with same pd target
		...
		pos_floatingBaseTask        = np.array([0.05, -0.1, 1.0]) # overridden inside wrapper
	...
	def step_simulation(self):
			...
			# self.tsc.updateStateWrapper(q_raw, dq_raw, acc_raw)									# Origional wrapper
			self.tsc.updateStateWrapper(q_raw, dq_raw, acc_raw, self.iter_step_increment)			# Zhaoyang's wrapper
	...
	def step(self, action_rl):
			heading_angle = action_rl[3]
			# desired_pos_x = self.qpos[0]
			# desired_pos_y = self.qpos[1]
			desired_pos_x = action_rl[4]
			desired_pos_y = action_rl[5]
			desired_vel_x = action_rl[6]
			# desired_vel_x = 0.5
			desired_vel_y = action_rl[7]
			# desired_vel_y = 0
			if self.iter > self.stand_time*1000:
				...
				pos_floatingBaseTask[0] = desired_pos_x
				pos_floatingBaseTask[1] = desired_pos_y
				pos_floatingBaseTask[2] = 1.0 
				...
				# no velocity requirement, can change vel_ and desired to com.vel.
				...
				desired_velTask[0] 		= desired_vel_x
				vel_floatingBaseTask[0] = desired_vel_x
				desired_velTask[1] 		= desired_vel_y
				vel_floatingBaseTask[1] = desired_vel_y
				...
				self.pfx_rel = action_rl[0]
				self.pfy_rel = action_rl[1]
				self.pfz_rel = 0 #0.3*action_rl[2]
				self.swfoot_clearance = 0.12
				terrain_angle = 0.0
				self.tsc.updateSwingFootTarget(self.pfx_rel, self.pfy_rel, self.pfz_rel, 
				   							   terrain_angle, self.swfoot_clearance)
	...
	def get_robo_glo_base_from_sensor(self):
		pos = np.array(self.data.sensordata[0:2])                              
		# quat = np.array(self.data.sensordata[3:7])						
		vel = np.array(self.data.sensordata[36:38])
		return pos, vel
	
	def get_fot_loc_com_from_sensor(self):
		m = 40
		H = 1.0
		pos = self.tsc.getCoMPos()
		vel = self.tsc.getCoMVel()
		L_com = self.tsc.getAngularMomentum()
		L_stf = L_com + m * np.cross(pos, vel)
		Ly_stf = L_stf[1]/(m*H) # normalized contact angular momentum
		Lx_stf = -L_stf[0]/(m*H)
		vel_ang = [Ly_stf, Lx_stf, 0.0]
		quat = np.array(self.data.sensordata[3:7])						
		return pos, vel_ang, quat
	
	def get_fot_loc_angm(self):
		m = 40
		x_alip = np.zeros(2)
		y_alip = np.zeros(2)
		pos = self.tsc.getCoMPos()
		vel = self.tsc.getCoMVel()
		L_com = self.tsc.getAngularMomentum()
		L_stf = L_com + np.cross(pos, m * vel)
		x_alip[0] = pos[0]
		x_alip[1] = L_stf[1]
		y_alip[0] = pos[1]
		y_alip[1] = L_stf[0]
		return x_alip, y_alip
	...
	def get_ft_glo_from_sensor(self):
		...
		return pos_left_foot_world_coord[0:2], pos_right_foot_world_coord[0:2]
