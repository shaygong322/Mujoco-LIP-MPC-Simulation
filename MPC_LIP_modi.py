...
class MPCCBF:
    ...
    def gen_control_test(self, state, leg_ind, init_guess, plot = False, trajec = []):
        ...
        xk = self.init_state
        self.select_obs(xk)
        u, feasi = self.solveMPCCBF(xk, leg_ind, init_guess)           # array format
        p_list = []
        hd_list = []
        temp_pos_det = []
        xk_list = [np.ravel(xk)]
        ...
        for j in range(self.N):
            ...
            temp_pos_det.append(self.xk_track_det(xk, p, self.dt)) 
        pos_det = np.concatenate(temp_pos_det)
        ...
        return xk_list[1:], p_list[0], hd_list, close_2_goal, feasi, pos_det
    ...
    def solveMPCCBF(self, xk, od_ev, init_guess):
        ...
        if od_ev > 0:
            for i in range(self.N):
                if np.mod(i, 2) == 0:
                    ...
                    cl = np.append(cl, np.zeros(len(self.sel_cir)))
                    cl = np.append(cl, np.zeros(len(self.sel_elp)))
                    cl = np.append(cl, [0, -self.ang_max, self.bvx_min])
                    cu = np.append(cu, [self.bvx_max, self.bvy_max])
                    cu = np.append(cu, np.inf*np.ones(len(self.sel_cir)))
                    cu = np.append(cu, np.inf*np.ones(len(self.sel_elp)))
                    cu = np.append(cu, [self.leg, self.ang_max, self.bvx_max])
                else:
        ...
        for i in range(len(self.sel_cir)):
            cir = self.sel_cir[i]
        ...
        nlp = cyipopt.Problem(
           ...
           problem_obj=LIP_Prob(xk, self.M_A, self.M_B, self.A, self.W, self.dx_du,
                                self.dP_du, self.sel_cir, self.sel_elp, goal, self.N),
           lb=lb,
           ub=ub,
           cl=cl,
           cu=cu,
        ...
        # nlp.get_current_violations(True)
        u, info = nlp.solve(u0)
        feasi = info['status']
        # if feasi == 2:
        #     print('num cir', len(self.sel_cir))
        #     print('num elp', len(self.sel_elp))
        #     print(info)
        return u, feasi
...
class LIP_Prob:
    def __init__(self, xk, M_A, M_B, A, W, dx, dp, cir_safe, elp_safe, goal, step):
        # MPC param
        self.p = 0
        self.q = 1
        self.r = 50
    ...
    def constraints(self, u):
        ...
        for i in range(self.N):
            ...
            d_ang = float(p[2, i])
            f_en = float(self.s*abs(d_ang)+v_body[0])
    ...
    def jacobian(self, u):
        ...
        dCBF1_cir = np.zeros([r, 15])
        dCBF2_cir = np.zeros([r, 15])
        dCBF3_cir = np.zeros([r, 15])
        for each in self.cir_modi_pram:
            dh1d1, dh1d2 = self.dh_cir(each, x[0, 1], x[1, 1])
            dh1_du = np.ravel(dh1d1*self.dx_du[5, :] + dh1d2*self.dx_du[6, :])
            dh2d1, dh2d2 = self.dh_cir(each, x[0, 2], x[1, 2])
            dh2_du = np.ravel(dh2d1*self.dx_du[10,:] + dh2d2*self.dx_du[11,:])
            dh3d1, dh3d2 = self.dh_cir(each, x[0, 3], x[1, 3])
        ...
        dCBF1_elp = np.zeros([r, 15])
        dCBF2_elp = np.zeros([r, 15])
        dCBF3_elp = np.zeros([r, 15])
        for each in self.elp_modi_pram:
            dh1d1, dh1d2 = self.dh_elp(each, x[0, 1], x[1, 1])
            dh1_du = np.ravel(dh1d1*self.dx_du[5, :] + dh1d2*self.dx_du[6, :])
            dh2d1, dh2d2 = self.dh_elp(each, x[0, 2], x[1, 2])
            dh2_du = np.ravel(dh2d1*self.dx_du[10,:] + dh2d2*self.dx_du[11,:])
            dh3d1, dh3d2 = self.dh_elp(each, x[0, 3], x[1, 3])
            dh3_du = np.ravel(dh3d1*self.dx_du[15,:] + dh3d2*self.dx_du[16,:])
        ...
        # Velo-turning-Jac
        dv_tn1 = self.den_du(p[2, 0], theta_con1, dv11_du)
        dv_tn2 = self.den_du(p[2, 1], theta_con2, dv12_du)
        dv_tn3 = self.den_du(p[2, 2], theta_con3, dv13_du)
        
        jac = np.concatenate([dv11_du, dv21_du, dCBF1_cir, dCBF1_elp, df1, theta_con1, dv_tn1, 
                              dv12_du, dv22_du, dCBF2_cir, dCBF2_elp, df2, theta_con2, dv_tn2, 
                              dv13_du, dv23_du, dCBF3_cir, dCBF3_elp, df3, theta_con3, dv_tn3])
        return jac
